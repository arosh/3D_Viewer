...
public class MeshEditor {
	...
	static public void smooth(final CustomTriangleMesh c, final float K) {
		...
		final Map<Point3f, PointGroup> ht = new HashMap<Point3f, PointGroup>();
	...
	static private class PointGroup {
		Point3f first;
		Set<Point3f> edges = new HashSet<Point3f>();
		List<Point3f> members = new ArrayList<Point3f>(); // can't be a HashSet, because points will compare as equal
		float vx, vy, vz;
		...
		void applyVector(Map<Point3f, PointGroup> ht) {
			// compute average displacement vector for all neighbors, i.e. edge points
			float ax=0, ay=0, az=0;
			int count = 0;
			for (Point3f edge : edges) {
				PointGroup pg = ht.get(edge);
				if (null == pg) continue;
				count++;
				ax += pg.vx;
				ay += pg.vy;
	...
	/** Build a list of points that are just one edge away from p1, and store them in the Map.  */
	static private void build(Point3f p1, Point3f p2, Point3f p3, Map<Point3f, PointGroup> ht) {
